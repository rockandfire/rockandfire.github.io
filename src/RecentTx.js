...
const createCards = async () => {
    ...
    try{
		...
		//map array to cards based on id
		//conditionally render buttons for buy and sell based on account
		...
		//render in returned element
        const root = createRoot(document.getElementById("value"));
        root.render(cards);
    }
    catch(error) {
        // it freaks out about the request rate without really affecting much
    }
}
...
//purchase nft given token and price
//price is not passed from frontend input
//price is also required to be >= cost in contract
async function buyNFT(tokenId, price) {
	getAccount();
	const transaction = contract.methods.buyNFT(tokenId);
